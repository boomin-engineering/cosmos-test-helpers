...
namespace CosmosTestHelpers.IntegrationTests
{
    ...
    public sealed class CosmosQueryEquivalencyTestsLegacyQueries : IAsyncLifetime, IDisposable
    {
        ...
        public async Task GivenAQueryUsingContainsOnAnEnumWithAToStringWhenExecutingThenTheResultsShouldMatch()
        {
            ...
            var (realResults, testResults) = _testCosmos.WhenExecutingAQuery<TestModel>(q => q.Where(tm => new[] { TestEnum.Value2.ToString() }.Contains(tm.EnumValue.ToString())));
        ...
        public async Task GivenAQueryUsingAnyInASubQueryWhenExecutingThenTheResultsShouldMatch()
        {
            ...
            var (realResults, testResults) = _testCosmos.WhenExecutingAQuery<TestModel>(q => q.Where(tm => tm.Children.Any(c => c.Value == "bob bobertson")));
        ...
        public async Task GivenAQueryUsingToUpperInvariantWhenExecutingThenBothShouldError()
        {
            ...
            var (realResults, testResults) = _testCosmos.WhenExecutingAQuery<TestModel>(q => q.Where(tm => tm.Name.ToUpperInvariant() == "BOB BOBERTSON"));
        ...
        public async Task GivenAQueryUsingToLowerInvariantWhenExecutingThenBothShouldError()
        {
            ...
            var (realResults, testResults) = _testCosmos.WhenExecutingAQuery<TestModel>(q => q.Where(tm => tm.Name.ToLowerInvariant() == "bob bobertson"));
