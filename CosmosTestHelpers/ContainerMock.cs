...
public class ContainerMock : Container
{
    ...
    public delegate Task DocumentChangedEvent(IReadOnlyCollection<object> changes, CancellationToken token);

    public event DocumentChangedEvent DocumentChanged
    {
        add => _containerData.DocumentChanged += value;
        remove => _containerData.DocumentChanged -= value;
    }

    public ContainerMock(string partitionKeyPath = null, UniqueKeyPolicy uniqueKeyPolicy = null, string containerName = "TestContainer", int defaultDocumentTimeToLive = -1)
    {
    ...
    )
    {
        ...
        if (JsonHelpers.GetIdFromJson(json) == string.Empty)
        {
            throw new CosmosException("Response status code does not indicate success: BadRequest (400);", HttpStatusCode.BadRequest, 400, Guid.NewGuid().ToString(), 0);
        }
    ...
    public override async Task<ItemResponse<T>> UpsertItemAsync<T>(T item, PartitionKey? partitionKey = default, ItemRequestOptions requestOptions = null, CancellationToken cancellationToken = default)
    {
        ThrowNextExceptionIfPresent(new InvocationInformation(nameof(UpsertItemAsync)));
    ...
    public override async Task<ItemResponse<T>> ReplaceItemAsync<T>(T item, string id, PartitionKey? partitionKey = default, ItemRequestOptions requestOptions = null, CancellationToken cancellationToken = default)
    {
